...
struct ddMatMulFun {
    INDEXTYPE stride;
    __device__
    float operator()( const float* data, INDEXTYPE x, INDEXTYPE y, float* src, int srcx){
        float m_ij = data[y*stride + x];
        float res;
        float a = src[srcx];
        res = m_ij * a;
        return res;
...
int backup_matmul(void* matctx, void* src, void* dst){
    ...
    ddStoreFun<int, float> storef;
    ddSumFun dsumf;
    matmulf.stride = mat->stride;
    error = (int)callFullMatMul<float>(m, matmulf, dsumf, storef, size, size, x0, y, false, 0, true);
#endif
    return error;
}
